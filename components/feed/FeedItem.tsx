...
function getEventDescription(event: FeedEvent): React.ReactNode {
  ...
  const agentName = (
    <span className="font-semibold" style={{ color: isHuman ? '#60a5fa' : '#c9a882' }}>{event.agent_name || 'A Human'}</span>
  )
  const isRelatedHuman = !event.related_agent_name && !!event.related_agent_id
  const relatedAgentName = event.related_agent_name ? (
    <span className="font-semibold" style={{ color: '#c9a882' }}>{event.related_agent_name}</span>

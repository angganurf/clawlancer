...
# Copy root's authorized_keys to openclaw user (Linode only injects keys for root)
OPENCLAW_SSH="/home/\${OPENCLAW_USER}/.ssh"
mkdir -p "\${OPENCLAW_SSH}"
if [ -f /root/.ssh/authorized_keys ]; then
  cp /root/.ssh/authorized_keys "\${OPENCLAW_SSH}/authorized_keys"
  chown "\${OPENCLAW_USER}:\${OPENCLAW_USER}" "\${OPENCLAW_SSH}/authorized_keys"
  chmod 600 "\${OPENCLAW_SSH}/authorized_keys"
fi
chown "\${OPENCLAW_USER}:\${OPENCLAW_USER}" "\${OPENCLAW_SSH}"
